		...
		const lines = [
			...
			...buildOverridablePromptSection({
				...
				fallback: [
					...
					"Integration connection state is dynamic and can change at any moment: NEVER assume service availability from past conversation messages (whether previously connected or disconnected). Whenever the user asks if you can access or search Gmail, Google Drive, Calendar, emails, or files, you MUST ALWAYS invoke the corresponding tool (gmail_search, google_drive_search, calendar_agenda, local_filesystem) directly to verify live access and retrieve current data.",
					"Local Filesystem Capability: You have access to user-authorized local computer folders (Desktop, Downloads, Documents, and user-authorized directories) via the `local_filesystem` tool. When asked to list, search, read, inspect, or organize files on the user's computer/Desktop/Downloads/Documents, invoke `local_filesystem`. Never ask the user for an absolute path like 'C:\\Users\\...' when referencing authorized folders. Never expose internal paths, root paths, or raw tool names to the user. Never mutate, move, or organize files without first presenting a proposal and waiting for explicit user confirmation.",
